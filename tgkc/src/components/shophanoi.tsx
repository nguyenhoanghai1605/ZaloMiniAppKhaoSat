...
interface shophanoiProps {
  layout: "cover" | "list-item";
  shophanoi: Restaurant;
  before?: React.ReactNode;
  after?: React.ReactNode;
  onClick?: (e: React.MouseEvent<HTMLDivElement>) => void;
}

const shophanoiItem: FunctionComponent<shophanoiProps> = ({
  layout,
  shophanoi,
  before,
  after,
  ...
  if (layout === "cover") {
    return (
      <div
        ...
        className="relative bg-white rounded-xl overflow-hidden p-0 shophanoi-with-cover"
      >
        ...
        <Box flex mt={0} mb={2}>
          ...
          >
            <span className="text-gray-500">
              <DistrictName id={shophanoi.districtId} />
            </span>
          </Button>
          <Button
          ...
          >
            <span className="text-gray-500">
              <Distance location={shophanoi.location} />
            </span>
          </Button>
        </Box>
      </div>
  ...
  return (
    <div
      onClick={onClick ?? viewDetail}
      className="bg-white rounded-xl overflow-hidden p-0 shophanoi-with-cover"
    >
      <Box m={0} flex>
        ...
        <Box my={4} mx={5} className="min-w-0">
          {before}
          <Title size="small">{shophanoi.name}</Title>
          {after}
          <Box mx={0} mb={0} flex>
            <Button
              prefixIcon={<Icon icon="zi-send-solid" />}
