...
function render(_ctx, _cache) {
  var _component_zmp_use_icon = _resolveComponent("zmp-use-icon");
  ...
  }), [_ctx.icon ? (_openBlock(), _createBlock(_component_zmp_use_icon, {
    key: 0,
    icon: _ctx.icon
  }, null, 8, ["icon"])) : _createCommentVNode("", true), _ctx.text ? (_openBlock(), _createBlock("span", {
    key: 1,
    class: _ctx.isTabbarLabel ? 'tabbar-label' : ''
  }, [_createTextVNode(_toDisplayString(_ctx.text) + " ", 1), _ctx.badge ? (_openBlock(), _createBlock(_component_zmp_badge, {
    key: 0,
...
import { useLink } from '../shared/use-link';
import { useIcon } from '../shared/use-icon';
import { useRouteProps } from '../shared/use-route-props';
import { useTooltip } from '../shared/use-tooltip';
import { useSmartSelect } from '../shared/use-smart-select';
import zmpBadge from '../components/badge';
import zmpUseIcon from '../components/use-icon';
export default {
  name: 'zmp-link',
  render: render,
  components: {
    zmpBadge: zmpBadge,
    zmpUseIcon: zmpUseIcon
  },
  props: _extends({
    noLinkClass: Boolean,
    text: String,
  ...
  setup: function setup(props, _ref) {
    ...
    var defaultHref = computed(function () {
      var _zmp$params$clicks$de, _zmp$params$clicks;

      return (_zmp$params$clicks$de = (_zmp$params$clicks = zmp.params.clicks) == null ? void 0 : _zmp$params$clicks.defaultHref) != null ? _zmp$params$clicks$de : '#';
    });
    ...
    var paths = computed(function () {
      var _zmp$routes, _zmp$views$routes, _zmp$views;

      return ((_zmp$routes = zmp.routes) != null ? _zmp$routes : []).concat((_zmp$views$routes = (_zmp$views = zmp.views[zmp.views.length - 1]) == null ? void 0 : _zmp$views.routes) != null ? _zmp$views$routes : []).map(function (route) {
        return route.path;
      });
    });
    var disabledLink = computed(function () {
      if (props.href && props.href !== '#' && !validateUrl(props.href, paths.value)) {
        return true;
      }
